...
int spg(int n, VERTEX *v, double **X, SEARCH S, OPTION op, INFORMATION *info, int *its, double *obj)
{
   ...
   while (maxIt > it && objval > op.epsobj && alpha > op.epsalpha)
   {
      ...
      {
         differenceMatrix(K, n, S.gX, S.gXp, S.YX);
         differenceVector(m, S.gy, S.gyp, S.Yy);
         differenceMatrix(K, n, X, S.Xp, S.ZX);
         differenceVector(m, S.y, S.yp, S.Zy);
      ...
      alpha = 2.0;
      copyMatrix(K, n, X, S.Xp);
      copyVector(m, S.y, S.yp);
      copyMatrix(K, n, S.gX, S.gXp);
      copyVector(m, S.gy, S.gyp);
      ...
      do
      {
         ...
         {
            for (k = 0; k < K; k++)
               X[k][i] = S.Xp[k][i] + alpha * S.DX[k][i];
         };
         for (j = 0; j < m; j++)
            S.y[j] = S.yp[j] + alpha * S.Dy[j];

         newobjval = compute_stress(n, v, X, S.y);
      } while (alpha > op.epsalpha && newobjval > C + op.gam * alpha * scalprod);

      if (alpha <= op.epsalpha)
         scalprod = scalprod / (norm(n, S.gX, m, S.gy) * norm(n, S.DX, m, S.Dy));
      newobjval = compute_stress(n, v, X, S.y);
