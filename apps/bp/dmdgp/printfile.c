...
void printfile(int n, VERTEX *v, double **X, char *filename, int s)
{
   ...
   groupId = v[0].groupId;
   for (i = 1; i < n && !group; i++)
      group = group || (groupId != v[i].groupId);
   ...
   for (i = 0; i < n; i++)
   {
      ...
      if (strcmp(v[i].Name, "(no name)"))
         fprintf(output, " %s", v[i].Name);

      // groupId
      if (group)
         fprintf(output, " %d", v[i].groupId);

      // Group
      if (strcmp(v[i].Group, "(no group name)"))
         fprintf(output, " %s", v[i].Group);
