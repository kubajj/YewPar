...
REFERENCE *getReference(VERTEX *v, int i, int j)
{
   ...
   ref = v[J].ref;
   while (ref != NULL && otherVertexId(ref) != I)
      ref = ref->next;
...
int isDDGP(int n, VERTEX *v, double eps, bool clique)
{
   ...
   {
      // all distances
      if (numberOfDistances(v[i].ref) < 3)
         return i;

      // exact distances
      if (numberOfExactDistances(v[i].ref, eps) < 2)
         return i;
   };
...
bool isDMDGP(int n, VERTEX *v, double eps, bool ddgp)
{
   ...
   if (!ddgp)
      if (isDDGP(n, v, eps, true) != 0)
         return false;

   // checking the consecutivity assumption
   for (i = 3; i < n; i++)
   {
      r3 = getReference(v, i - 3, i);
      if (r3 == NULL)
         return false;
      r2 = getReference(v, i - 2, i);
      if (r2 == NULL)
         return false;
      r1 = getReference(v, i - 1, i);
      if (r1 == NULL)
         return false;
      if (isIntervalDistance(r1, eps) || isIntervalDistance(r2, eps))
         return false;
   };
...
void findSymmetries(int n, VERTEX *v, bool *sym)
{
   ...
   for (i = 0; i < n; i++)
   {
      ...
      while (ref != NULL)
      {
         for (j = otherVertexId(ref) + 4; j <= i; j++)
            sym[j] = false;
         ref = ref->next;
      };
   };
...
bool isValidTriplet(triplet t, double eps)
{
   ...
   if (t.r3 == t.r1)
      return false;
   if (isExactDistance(t.r1, eps))
      count++;
   if (isExactDistance(t.r2, eps))
      count++;
   if (isExactDistance(t.r3, eps))
      count++;
   return count >= 2;
};
...
triplet nextTripletRef(REFERENCE *ref, triplet t, double eps)
{
   ...
   if (isNullTriplet(t))
   {
      ...
      if (isNullTriplet(t))
         return nt; // no triplets exist
      if (isValidTriplet(t, eps))
         return t; // found already!
   };
...
bool isExactClique(int id, VERTEX *v, triplet t, double eps, double *cosangle)
{
   ...
   if (ij == NULL)
      return false;
   if (!isExactDistance(ij, eps))
      return false;
   jk = getReference(v, j, k);
   if (jk == NULL)
      return false;
   if (!isExactDistance(jk, eps))
      return false;
   ik = getReference(v, i, k);
   if (ik == NULL)
      return false;
   if (!isExactDistance(ik, eps))
      return false;
...
triplet findReferencesExactCase(int id, VERTEX *v, double eps, double *cosine)
{
   ...
   if (isExactClique(id, v, refs, eps, cosine))
      return refs;
   return nullTriplet();
};
