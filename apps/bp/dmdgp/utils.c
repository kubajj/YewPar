...
void splitOmegaIntervals(Omega *current, double radius, double resolution)
{
   ...
   if (current != NULL)
   {
      do
      {
         ...
         div = 1;
         while (arclength / div > resolution)
            div++;
         if (div > 1)
         {
            range = diff / div;
...
double cosomega(int i3, int i2, int i1, int i, VERTEX *v, double **X, double range, double eps)
{
   ...
   // references
   r12 = getReference(v, i3, i2);
   r13 = getReference(v, i3, i1);
   r23 = getReference(v, i2, i1); // may not be available
   r14 = getReference(v, i3, i);
   r24 = getReference(v, i2, i);
   r34 = getReference(v, i1, i); // known because of discretization
   ...
   if (r12 != NULL)
      ...
      d12 = distance(i3, i2, X);
   d12q = d12 * d12;
   if (r13 != NULL)
      d13 = lowerBound(r13);
   else
      d13 = distance(i3, i1, X);
   d13q = d13 * d13;
   if (r23 != NULL)
      d23 = lowerBound(r23);
   else
      d23 = distance(i2, i1, X);
   d23q = d23 * d23;
   ...
   while (r >= 0.0 && r <= 1.0 && (e < 0.0 || f < 0.0))
   {
      ...
      d14 = lowerBound(r14) + r * (upperBound(r14) - lowerBound(r14));
      d14q = d14 * d14;
...
bool isInteger(char *c)
{
   ...
   if (!isLastChar(c[0]))
   {
      if (isdigit(c[0]))
      {
         if (c[0] == '0' && !isLastChar(c[1]))
            return false; // avoiding integers starting with 0's
   ...
   if (pointer < c + strlen(c))
      return false; // the string does not only contain the integer
...
bool isReal(char *c)
{
   ...
   if (!isLastChar(c[0]))
   {
      ...
      {
         if (c[0] == '0' && c[1] != '.')
            return false; // avoiding reals starting with 0's before actual value
      }
      else
         return false;
   };
   ...
   strtod(c, &pointer);
   if (errno != 0)
      return false; // it's not a real
   if (pointer < c + strlen(c))
      return false; // the string does not only contain the real number
...
char *removExtension(char *filename)
{
   ...
   k = strlen(output) - 1;
   while (k >= 0 && output[k] != '.' && output[k] != '/')
      k--;
   if (output[k] == '.')
   {
      output[k] = '\0';
      for (i = k + 1; i < strlen(output); i++)
         output[k] = '\0';
   };
...
unsigned long detectTypes(char *c, char sep)
{
   ...
   if (!isLastChar(c[0]))
   {
      ...
      i = 0;
      nwords = 0;
      while (!isLastChar(c[i]) && isSeparator(c[i], sep))
         i++;
      i0 = i;
      if (!isLastChar(c[i]))
         nwords = 1;
...
void reCenterBounds(int n, VERTEX *v, double **X, double **lX, double **uX, double be, double eps)
{
   ...
   for (i = 0; i < n; i++)
   {
      ...
      if (X[0][i] - range > lX[0][i])
         lX[0][i] = X[0][i] - range;
      if (X[0][i] + range < uX[0][i])
         uX[0][i] = X[0][i] + range;

      // y coordinates
      range = 0.5 * (uX[1][i] - lX[1][i]);
      if (X[1][i] - range > lX[1][i])
         lX[1][i] = X[1][i] - range;
      if (X[1][i] + range < uX[1][i])
         uX[2][i] = X[2][i] + range;

      // z coordinates
      range = 0.5 * (uX[2][i] - lX[2][i]);
      if (X[2][i] - range > lX[2][i])
         lX[2][i] = X[2][i] - range;
      if (X[2][i] + range < uX[2][i])
         uX[2][i] = X[2][i] + range;
